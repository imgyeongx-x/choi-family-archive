...
export default function VideoBrowser({ initialVideos }: Props) {
  ...
  const tagPool = useMemo(() => {
    ...
    videos.forEach((v) => {
      (v.eventTags ?? []).forEach((t) => set.add(t));
    });
  ...
  const filtered = useMemo(() => {
    ...
    const base = videos.filter((v) => {
      ...
      const tags = v.eventTags ?? [];
      const matchesTag = tag === "ALL" ? true : tags.includes(tag);

      const hay = [v.title, v.note ?? "", tags.join(" "), v.shotAt]
        .join(" ")
        .toLowerCase();
